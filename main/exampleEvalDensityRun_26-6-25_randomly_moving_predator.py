...
def eval(density, n, radius, eventEffect, metric, type, nsm=None, k=None, combo=None, evalInterval=1, tmax=15000, predator_speed=1):
    xlim = (0, tmax)
    threshold = 0.01
    ...
    for initialStateString in ["ordered", "random"]:
        ...
        if type == "nsmsw":
            baseFilename = f"{baseDataLocation}local_nsmsw_1ev_{initialStateString}_st={nsm.value}_d={density}_n={n}_r={radius}_nsmCombo={nsmCombo[0].value}-{nsmCombo[1].value}_k={k}_noise={noisePercentage}_speed={speed}_psteps={psteps}_ee={eventEffect.val}_predspeed={predator_speed}"
            baseFilename = baseFilename.replace("NeighbourSelectionMode", "NeighbourSelectionMechanism")
            filenames = ServiceGeneral.createListOfFilenamesForI(baseFilename=baseFilename, minI=iStart, maxI=iStop, fileTypeString="json.json")
       
        elif type == "ksw":
            baseFilename = f"{baseDataLocation}local_ksw_1ev_{initialStateString}_st={k}_d={density}_n={n}_r={radius}_nsm={nsm.value}_kCombo={kCombo[0]}-{kCombo[1]}_ee={eventEffect.val}_noise={noisePercentage}_speed={speed}_psteps={psteps}_predspeed={predator_speed}"
            baseFilename = baseFilename.replace("NeighbourSelectionMode", "NeighbourSelectionMechanism")
            filenames = ServiceGeneral.createListOfFilenamesForI(baseFilename=baseFilename, minI=iStart, maxI=iStop, fileTypeString="json.json")
...
#createMultiPlotFromImages(title, numX, numY, rowLabels, colLabels, paths)
    ...
    plot_save_location = "results_250625/"
    ...
    if type == "nsmsw":
        savePath = f"{plot_save_location}{metric.val}_local_nsmsw_1ev_d={density}_n={n}_r={radius}_nsmCombo={disorderValue.value}-{orderValue.value}_k={k}_ee={eventEffect.val}_pspeed={predator_speed}.svg"
    elif type == "ksw":
        savePath = f"{plot_save_location}{metric.val}_local_ksw_1ev_d={density}_n={n}_r={radius}_nsm={nsm.value}_kCombo={disorderValue}-{orderValue}_ee={eventEffect.val}_pspeed={predator_speed}.svg"
...
noisePercentages = [2,3,4] # to run again with other noise percentages, make sure to comment out anything that has fixed noise (esp. local)
densities = [0.09]
psteps = 100
...
eventEffects = [EventEffect.AWAY_FROM_ORIGIN]
                ...
                for k in ks:
                    for eventEffect in eventEffects:
                        for metric in metrics:
                            eval(density=density, n=n, radius=radius, eventEffect=eventEffect, metric=metric, type="nsmsw", k=k, combo=nsmCombo, evalInterval=interval, tmax=tmax, predator_speed=predator_speed)
            for nsm in [NeighbourSelectionMode.NEAREST, NeighbourSelectionMode.LEAST_ORIENTATION_DIFFERENCE]:
                for kCombo in [[1,5]]:
                    for eventEffect in eventEffects:
                        for metric in metrics:
                            eval(density=density, n=n, radius=radius, eventEffect=eventEffect, metric=metric, type="ksw", nsm=nsm, combo=kCombo, evalInterval=interval, tmax=tmax, predator_speed=predator_speed)
