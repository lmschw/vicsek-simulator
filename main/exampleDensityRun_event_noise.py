...
saveLocation = "J:/window_size_check/"
iStart = 1
iStop = 11
                        ...
                        for initialStateString in ["ordered", "random"]:
                            ...
                            for eventEffectOrder in eventEffects:
                                for i in range(iStart,iStop):
                                    ...
                                    savePath = f"{saveLocation}local_1e_nosw_{initialStateString}_st={startValue.value}__d={density}_n={n}_r={radius}_k={k}_noise={noisePercentage}_speed={speed}_drn={duration}_psteps={psteps}_{eventsString}_{i}"
                                ...
                                for initialStateString in ["ordered", "random"]:
                                    ...
                                    for eventEffectOrder in eventEffects:
                                        for i in range(iStart,iStop):
                                            ...
                                            # Save model values for future use
                                            eventsString = f"{event1.timestep}-{event1.eventEffect.val}"
                                            savePath = f"{saveLocation}local_nsmsw_1ev_{initialStateString}_st={startValue.value}_d={density}_n={n}_r={radius}_nsmCombo={nsmCombo[0].value}-{nsmCombo[1].value}_k={k}_noise={noisePercentage}_speed={speed}_psteps={psteps}_ee={eventEffectOrder.val}_{i}.json"
                                            ServiceSavedModel.saveModel(simulationData=simulationData, colours=colours, switchValues=switchValues, path=f"{savePath}.json", modelParams=simulator.getParameterSummary())
                    ...
                    for initialStateString in ["ordered", "random"]:
                        ...
                        for eventEffectOrder in eventEffects:
                            for i in range(iStart,iStop):
                                ...
                                # Save model values for future use
                                eventsString = f"{event1.timestep}-{event1.eventEffect.val}"
                                savePath = f"{saveLocation}local_ksw_1ev_{initialStateString}_st={startValue}_d={density}_n={n}_r={radius}_nsm={neighbourSelectionMode.value}_kCombo={kCombo[0]}-{kCombo[1]}_ee={eventEffectOrder.val}_noise={noisePercentage}_speed={speed}_psteps={psteps}_{i}.json"
                                ServiceSavedModel.saveModel(simulationData=simulationData, colours=colours, switchValues=switchValues, path=f"{savePath}.json", modelParams=simulator.getParameterSummary())
