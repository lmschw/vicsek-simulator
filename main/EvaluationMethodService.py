...
def evaluateAllModes(n, k, noise, radius, metric=EnumMetrics.Metrics.ORDER):
    modelParams, simulationData = SavedModelService.loadModels([f"examples/RANDOM_n={n}_k={k}_noise={noise}_radius={radius}.json",
...
def evaluateN(mode, k, noise, radius, metric=EnumMetrics.Metrics.ORDER):
    modelParams, simulationData = SavedModelService.loadModels([f"examples/{mode.name}_n=100_k={k}_noise={noise}_radius={radius}.json",
...
def evaluateK(mode, n, noise, radius, metric=EnumMetrics.Metrics.ORDER):
    modelParams, simulationData = SavedModelService.loadModels([f"examples/{mode.name}_n={n}_k=1_noise={noise}_radius={radius}.json",
...
def evaluateNoise(mode, n, k, radius, metric=EnumMetrics.Metrics.ORDER):
    modelParams, simulationData = SavedModelService.loadModels([f"examples/{mode.name}_n={n}_k={k}_noise=0_radius={radius}.json",
...
def evaluateRadius(mode, n, k, noise, metric=EnumMetrics.Metrics.ORDER):
    modelParams, simulationData = SavedModelService.loadModels([f"examples/{mode.name}_n={n}_k={k}_noise={noise}_radius=10.json",
