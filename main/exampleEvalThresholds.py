...
def eval(density, n, radius, eventEffect, metric, type, switch_threshold, nsm=None, k=None, combo=None, evalInterval=1, tmax=15000):
    xlim = (0, tmax)
    threshold = 0.01
    ...
    for initialStateString in ["ordered", "random"]:
        ...
        if type == "nsmsw":
            baseFilename = f"{baseDataLocation}{levelDataLocation}local_nsmsw_1ev_{initialStateString}_st={nsm.value}_d={density}_n={n}_r={radius}_nsmCombo={disorderValue.value}-{orderValue.value}_k={k}_noise={noisePercentage}_speed={speed}_ee={eventEffect.val}_threshold={switch_threshold}"
        elif type == "ksw":
            baseFilename = f"{baseDataLocation}{levelDataLocation}local_ksw_1ev_{initialStateString}_st={k}_d={density}_n={n}_r={radius}_nsm={nsm.value}_kCombo={disorderValue}-{orderValue}_ee={eventEffect.val}_noise={noisePercentage}_speed={speed}_threshold={switch_threshold}"
...
#createMultiPlotFromImages(title, numX, numY, rowLabels, colLabels, paths)
    ...
    if type == "nosw":
        savePath = f"{saveLocation}{metric.val}_d={density}_n={n}_r={radius}_nosw_nsm={nsm.value}_k={k}_ee={eventEffect.val}_th={switch_threshold}.svg"
    elif type == "nsmsw":
        savePath = f"{saveLocation}{metric.val}_d={density}_n={n}_r={radius}_swt=MODE_o={orderValue.value}_do={disorderValue.value}_k={k}_ee={eventEffect.val}_th={switch_threshold}.svg"
    elif type == "ksw":
        savePath = f"{saveLocation}{metric.val}_d={density}_n={n}_r={radius}_swt=K_o={orderValue}_do={disorderValue}_nsm={nsm.value}_ee={eventEffect.val}_th={switch_threshold}.svg"
    elif type == "global":
        savePath = f"{saveLocation}{metric.val}_d={density}_n={n}_r={radius}_global_nsm={nsm.value}_k={k}_th={switch_threshold}.svg"
                ...
                for k in [1]:
                    for eventEffect in eventEffects:
                        for metric in metrics:
                            eval(density=density, n=n, radius=radius, switch_threshold=threshold, eventEffect=eventEffect, metric=metric, type="nsmsw", k=k, combo=nsmCombo, evalInterval=interval, tmax=tmax)
            
            for nsm in [NeighbourSelectionMode.NEAREST, NeighbourSelectionMode.LEAST_ORIENTATION_DIFFERENCE]:
                for kCombo in [[5,1]]:
                    for eventEffect in eventEffects:
                        for metric in metrics:
                            eval(density=density, n=n, radius=radius, switch_threshold=threshold, eventEffect=eventEffect, metric=metric, type="ksw", nsm=nsm, combo=kCombo, evalInterval=interval, tmax=tmax)
