...
class VicsekWithNeighbourSelection(VicsekWithNeighbourSelectionSwitchingCellBasedIndividualsDuration.VicsekWithNeighbourSelection):
    ...
    def __init__(self, neighbourSelectionModel, domainSize=dv.DEFAULT_DOMAIN_SIZE_2D, speed=dv.DEFAULT_SPEED, 
                 ...
                 kValues=[1,2], enforceSplit=True, probabilityK0=0.5, showExample=dv.DEFAULT_SHOW_EXAMPLE_PARTICLE, numCells=None, 
        ...
        self.enforceSplit = enforceSplit
        self.probabilityK0 = probabilityK0

    def prepareKs(self):
        if self.enforceSplit:
            numAffected = int(self.probabilityK0 * self.numberOfParticles)
            ...
            ks = np.where(rands <= self.probabilityK0/100, k0s, k1s)
        return ks
