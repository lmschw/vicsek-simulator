...
durations = [1, 50, 100, 500, 1000]
ks = [1, 5]
additionalKs = [0, 2, 3, 4, 10]
allKs = [0, 1, 2, 3, 4, 5, 10]
additionalKs = allKs
radii = [20] # area is always 4x bigger than the last
...
for density in densities:
    ...
    for radius in radii:
        ...
        for initialStateString in ["ordered", "random"]:
            for neighbourSelectionMode in neighbourSelectionModes:
                for k in additionalKs:
                    for i in range(iStart,iStop): 
                        ServiceGeneral.logWithTime(f"Starting ks d={density}, r={radius}, {initialStateString}, nsm={neighbourSelectionMode.name}, k={k} i={i}")
                        startRun = time.time()
            ...
            endTotal = time.time()
            ServiceGeneral.logWithTime(f"Completed GLOBAL ks in {ServiceGeneral.formatTime(endTotal-startTotal)}")

            
        
        # GLOBAL - RULE ONLY - NOISE COMPARISON
        tmax = tmaxWithoutEvent
        ...
        for noisePercentage in noisePercentages:
            ...
            for initialStateString in ["ordered", "random"]:
                for neighbourSelectionMode in neighbourSelectionModes:
                    for k in ks:
                        for i in range(iStart,iStop): 
                            ServiceGeneral.logWithTime(f"Starting noise d={density}, r={radius}, noise={noisePercentage}, {initialStateString}, nsm={neighbourSelectionMode.name}, k={k} i={i}")
                            startRun = time.time()
                            ...
                            endRun = time.time()
                            ServiceGeneral.logWithTime(f"Completed d={density}, r={radius}, noise={noisePercentage} {initialStateString}, nsm={neighbourSelectionMode.name}, k={k} i={i} in {ServiceGeneral.formatTime(endRun-startRun)}")
                endTotal = time.time()
...
# ----------------------------------------------- LOCAL STARTS HERE ----------------------------------------------
        ...
        """
        for combo in [(NeighbourSelectionMode.FARTHEST, NeighbourSelectionMode.NEAREST),
